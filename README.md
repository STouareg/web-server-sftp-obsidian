...
| `LOGO_LINK_URL` | No | [skinia.com.ua](https://www.skinia.com.ua/) | Link under the logo (opens in a new tab). Set **`LOGO_LINK_URL=`** (empty) to hide. |
| `LOGO_LINK_TEXT` | No | `Церква «Скинія»` | Label for that link (matches [Церква «Скинія»](https://www.skinia.com.ua/) site title). |
